...
// Hyperia lib-only build: only the local, network-free modules are compiled.
// The cloud/CLI modules (semantic_mesh, spelling, inversion, hybrid, cli) stay
// on disk for the upstream repo but are not declared here — they pull in ureq +
// shivvr.nuts.services and aren't needed for internal shell/sticky search.
pub mod bm25;
