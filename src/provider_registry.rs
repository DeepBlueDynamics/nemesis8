...
// Compiled-in provider TOMLs — available on the host binary even without the
// Docker image. The array is GENERATED by build.rs from providers/*.toml, so
// adding a provider file automatically embeds it (issue #34).
...
impl ProviderRegistry {
    ...
    /// Names of the compiled-in (builtin) providers, regardless of what's on
    /// disk. Drives the default install set: every provider TOML shipped in
    /// the repo is installed in the default image build.
